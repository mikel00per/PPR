...
void Equilibrado_Carga(tPila *pila, bool *fin, tNodo *solucion) {
  ...
  if (pila->vacia()) { // el proceso no tiene trabajo: pide a otros procesos
    /* Enviar petición de trabajo al proceso (idProceso + 1) % size */
    ...
    while (pila->vacia() && !*fin) {
      ...
      switch (status.MPI_TAG) {
        case PETICION: // peticion de trabajo
          ...
          /* Reenviar petición de trabajo al proceso (idProceso + 1) % size */
          MPI_Send(&solicitante, 1, MPI_INT, siguiente, PETICION, comunicadorCarga);
          if (solicitante == idProceso) { // peticion devuelta
      ...
      switch (status.MPI_TAG) {
        case PETICION:
          ...
          if (pila->tamanio() > 1) {
            ...
            delete pilaNueva;
            if (idProceso < solicitante) {
              color = NEGRO;
            }
          } else {
            /* Pasar petición de trabajo al proceso (idProceso + 1) % size */
            MPI_Send(&solicitante, 1, MPI_INT, siguiente, PETICION, comunicadorCarga);
          }
          break;
        case TOKEN:
...
void Difusion_Cota_Superior(int *U, bool *nueva_U) {
  ...
  if (difundir_cs_local && !pendiente_retorno_cs) {
    /* Enviar valor local de cs al proceos (idProceso + 1) % size */
    MPI_Send(U, 1, MPI_INT, siguiente, idProceso, comunicadorCota);
  ...
  while (hay_mensajes) {
    /* Recibir mensajes con valor de cota superior desde el proceso (idProceso - 1 + size) % size */
    ...
    if (status.MPI_TAG == idProceso) { // origen mensaje == propio proceso
      if (difundir_cs_local) {
        /* Enviar valor local de cs al proceso (idProceso + 1) % size */
        MPI_Send(U, 1, MPI_INT, siguiente, idProceso, comunicadorCota);
        pendiente_retorno_cs = true;
        difundir_cs_local = false;
      } else {
      ...
      /* Reenviar mensaje al proceso (idProceso + 1) % size */

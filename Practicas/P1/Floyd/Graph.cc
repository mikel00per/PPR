...
void Graph::obtenMasCorto() {
  ...
  cout << " ->  Más corto: ["<<i_f <<","<< j_f <<"]" << " cuya distancia es: " << A[i_f*vertices+j_f] << endl;
}
...
void Graph::obtenMasLargo() {
  ...
  for(i=0;i<vertices;i++)
    for(j=0;j<vertices;j++)
      if ((A[i*vertices+j] > A[i_f*vertices+j_f]) && (A[i*vertices+j] != 1000000) && (A[i*vertices+j] != INF) ){
        i_f = i; j_f = j;
      }

  cout << " ->  Más Largo: ["<<i_f <<","<< j_f <<"]" << " cuya distancia es: " << A[i_f*vertices+j_f] << endl;
}
